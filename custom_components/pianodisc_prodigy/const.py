...
# Current firmware publishes these slow-moving status topics retained.
...
# Retained {"audio":"x.y.z","midi":"x.y.z"} so MQTT-only installs (no HTTP
# IP) still learn the firmware versions that HTTP /debugJson otherwise provides.
MQTT_TOPIC_VERSION: Final = "version"
# Retained {"audio_latest","midi_latest","audio_url","midi_url"} — the latest
# available firmware, as reported by the piano's own update check.
MQTT_TOPIC_UPDATE: Final = "update"
# MQTT library/playlist request-response bridge, so Browse Media and playlist source
# discovery work without HTTP.
...
# .../ready carries the piano's readiness word (WARMING_UP, READY, NO_SD, FAULT),
# retained, so an HA restart reads it at once. The piano registers an MQTT Last-Will
# of "OFFLINE" (retained) so the broker announces an ungraceful disconnect.
...
# consistent with no completion signal).
...
# /songlist is a single shared buffer with NO completion signal, so the
# paged scan must POLL until the buffer advances to the *next* page — a fixed wait reads
# the stale buffer and silently truncates the library.
# Poll gently every SCAN_POLL_INTERVAL up to SCAN_POLL_MAX times per attempt,
...
# HA scans every page eagerly for a flat BROWSE_MEDIA list, so a 1s poll keeps the full
# scan ~30s while staying far gentler than the old 0.4s (which hammered the device). Advance is
...
# Volume scaling: device speaks 1..100, HA speaks 0..1.
# 0 is the mute volume; normal non-muted playback is 1..100.
VOLUME_MIN: Final = 0
VOLUME_MAX: Final = 100

# Polling cadence. In MQTT mode the HTTP poll is authoritative for state/volume, but
# heavy HTTP traffic disturbs playback — so poll LIGHTLY while playing (MQTT busy carries
# the real-time "playing" between polls). Aggressive polling makes playback choppy and
# the device's MQTT laggy.
...
# Reachable but state/volume not resolved yet (boot / ~1-min startup sync).
...
# piano ignores player commands before then) — but not forever.
...
# burst so the per-note flapping doesn't bounce the state.
...
# the fallback latest-version signal when the piano does not report update data
# itself. Actual installed versions are always displayed from device telemetry.
FW_AUDIO_FLOOR: Final = "0.5.0"
FW_AUDIO_RECOMMENDED: Final = "0.5.1"
FW_MIDI_FLOOR: Final = "1.4.0"
FW_MIDI_RECOMMENDED: Final = "1.4.5"
