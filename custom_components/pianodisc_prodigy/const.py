...
against real hardware (deviceID 0123456789AB) before locking.
"""
...
# Verified live 2026-05-29 against 0123456789AB; see .
...
# Current firmware makes these slow-moving status topics retained and truthful.
...
MQTT_TOPIC_SONG: Final = "song"  # raw UTF-8 title; cleared on Current firmware
...
# integration is public, so it can't hold a credential itself).
MQTT_TOPIC_UPDATE: Final = "update"
# : MQTT library/playlist request-response bridge. These remove the HTTP
...
# consistent with no completion signal — see ).
...
# Cadence + termination mirror the reference Calibrate app (, verified
...
# stops, waits POWER_OFF_SETTLE, then de-energizes. See .
...
# Firmware version policy (design). Floors/recommended.
# TODO: confirm the exact compat range.
